...
public sealed class LlamaRenderService : ILlamaRenderService
{
    ...
    // Place this near the top of LlamaRenderService, below the constants
    ...
    private async Task DrawArtAsync(Image<Rgba32> canvas, ICardDefinition card, CancellationToken cancellationToken)
    {
    ...
    private async Task DrawFramesAsync(Image<Rgba32> canvas, ICardDefinition card, CancellationToken cancellationToken)
    {
        if (card.Frames is null || !card.Frames.Any()) return;
        ...
        foreach (var frame in card.Frames.AsEnumerable().Reverse())
        {
            ...
            if (frame.Masks is not null && frame.Masks.Any())
            {
    ...
    private static float ResolveFontSize(ITextBlockDefinition text, int cardWidth)
    {
    ...
    private void EnsureFontsLoaded()
    {
        ...
        lock (_fontLock)
        {
            ...
            var roots = new[]
            {
                //Path.Combine(_engineConfig.ContentRootPath, "fonts"),
                Path.Combine(_engineConfig.WebRootPath, "fonts")
    ...
    private static (int Width, int Height) ResolveCanvasSize(ICardDefinition card)
    {
    ...
    private async Task<Image<Rgba32>?> LoadImageAsync(string source, CancellationToken cancellationToken)
    {
        ...
        // Handle data: URIs inline (no need to delegate to IAssetFetcher)
        ...
        // Delegate all other sources (HTTP, local paths, etc.) to IAssetFetcher
        try
        {
            await using var stream = await _assetFetcher.FetchAssetStreamAsync(source, cancellationToken);
    ...
    private string? ResolveLocalPath(string source)
    {
        ...
        if (!normalized.Contains('/') && !normalized.Contains('\\')) return EnsureInsideRoot(_engineConfig.LocalArtRoot, normalized);
...
// Inside ProcessTag()...
    ...
    private (float Width, float Height) MeasureAndDrawTokens(Image<Rgba32>? textLayer,
        ...
        bool measureOnly,
        ITextBlockDefinition textBlock,
        ICardDefinition card,
...
// 2. Apply the correct dynamic styling technique
    ...
    private void DrawText(Image<Rgba32> canvas, ICardDefinition card)
    {
...
// If it's the rules text on a special frame (Oil Slick, Mystical Archive, etc.), force top-align!
    ...
    private async Task DrawSetSymbolAsync(Image<Rgba32> canvas, ICardDefinition card, CancellationToken cancellationToken)
    {
    ...
    public async Task<RenderLayeredResult> RenderLayeredAsync(
        ICardDefinition card,
        bool preview,
        int? maxDimension,
        string? cardSizeProfileName,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        Configuration.Default.MemoryAllocator.ReleaseRetainedResources();
        return new RenderLayeredResult(artLayerStream, textLayerStream);
    }
    ...
    public class TextRenderState
    {
        ...
        public Font GetCurrentFont(LlamaRenderService service)
        {
