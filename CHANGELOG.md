...
### Added
- Extracted card rendering engine into standalone `LlamaMagic.Rendering` class library. Moved `LlamaRenderService` (formerly `CardRenderV2Service`), `Tokenizer`, `SvgRasterizationService`, `IPrepressService`/`PrepressService`, `PrepressPipelineOptions`, and all Sizing types (`CardSizeCatalog`, `CardSizeProfile`, `BleedAdder`, `MyImageExtensions`, `IBasicCardDimensions`). Introduced decoupling interfaces (`ICardDefinition`, `IFrameDefinition`, `IMaskDefinition`, `IBoundsDefinition`, `ITextBlockDefinition`) in `LlamaMagic.Rendering.Models`, plus `EngineConfiguration` record and `IAssetFetcher` abstraction in `LlamaMagic.Rendering.Infrastructure`. Web project models (`CardData`, `CardFrame`, `CardFrameMask`, `CardBounds`, `CardSymbolBounds`, `CardTextObject`) now implement these interfaces; `WebAssetFetcher` bridges `IAssetFetcher` to `IHttpClientFactory` + local file resolution.
...
- Renderer V2 set symbol (SVG): symbol was rendering too large and in the wrong position. C# now reads native SVG dimensions via `GetSvgNativeDimensions` and rasterizes at `nativeSize × zoom` — matching JS's `setSymbol.width * card.setSymbolZoom`. Explicit `setSymbolX/Y` (which already have alignment baked in by JS `resetSetSymbol`) are now used directly, not overridden by bounds math.
