...
### Added
- Extracted card rendering engine into standalone `LlamaMagic.Rendering` class library. Moved `LlamaRenderService` (formerly `CardRenderV2Service`), `Tokenizer`, `SvgRasterizationService`, and all Sizing types (`CardSizeCatalog`, `CardSizeProfile`, `BleedAdder`, `MyImageExtensions`, `IBasicCardDimensions`). Introduced decoupling interfaces (`ICardDefinition`, `IFrameDefinition`, `IMaskDefinition`, `IBoundsDefinition`, `ITextBlockDefinition`) in `LlamaMagic.Rendering.Models`, plus `EngineConfiguration` record and `IAssetFetcher` abstraction in `LlamaMagic.Rendering.Infrastructure`. Web project models (`CardData`, `CardFrame`, `CardFrameMask`, `CardBounds`, `CardSymbolBounds`, `CardTextObject`) now implement these interfaces; `WebAssetFetcher` bridges `IAssetFetcher` to `IHttpClientFactory` + local file resolution.
